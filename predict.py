...
def decoder(pred, threshold=0.1):
    '''
        ...
        for j in range(grid_num):
            for b in range(2):
                if mask[i, j, b] == 1:
                    ...
                    if float((contain_prob * max_prob)[0]) > threshold:
...
def predict_image(model, image_name, root_img_directory='/content/drive/My Drive/DL_Fall_2020/Assignment_3/VOCdevkit_2007/VOC2007test/JPEGImages/', threshold=0.1):
    """
    ...
    with torch.no_grad():
        ...
        pred = pred.cpu()
        boxes, cls_indexs, probs = decoder(pred, threshold)
